...
CREATE TABLE `risiko` (
  ...
  `id_master_risiko` int(11) DEFAULT NULL,
  `id_proyek` int(11) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

INSERT INTO `risiko` (`id_risiko`, `nilai_s`, `nama_kontrol`, `nilai_d`, `id_master_risiko`, `id_proyek`) VALUES
(1, 6, 'Tidak ada', 10, 1, 3),
(2, 7, 'Penjadwalan proyek', 1, 2, 3),
(3, 10, 'Pemberitahuan jadwal pembayaran ke owner', 2, 3, 3),
(4, 2, 'Tidak ada', 10, 4, 3),
(5, 7, 'Tidak ada', 8, 5, 3),
(6, 6, 'Pengawasan lapangan', 2, 6, 3),
(7, 5, 'Absensi pekerja', 1, 7, 3),
(8, 2, 'Hasil laporan', 8, 8, 3),
(9, 2, 'Hasil laporan', 8, 9, 3),
(10, 5, 'Hasil laporan', 6, 10, 3),
(11, 8, 'Memberi daftar tipe material yang dibeli', 7, 11, 3),
(12, 10, 'Hasil laporan', 6, 12, 3),
(13, 6, 'Hasil laporan', 8, 13, 3),
(14, 5, 'Tidak ada', 10, 14, 3),
(15, 9, 'Tidak ada', 10, 15, 3),
(16, 8, 'Tidak ada', 8, 16, 3);
...
ALTER TABLE `risiko`
  ...
  ADD KEY `id_master_risiko` (`id_master_risiko`),
  ADD KEY `id_proyek` (`id_proyek`);
...
ALTER TABLE `master_risiko`
  MODIFY `id_master_risiko` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=17;

ALTER TABLE `pekerjaan`
  MODIFY `id_pekerjaan` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=28;

ALTER TABLE `risiko`
  MODIFY `id_risiko` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=17;

ALTER TABLE `efek`
  MODIFY `id_efek` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=21;
...
ALTER TABLE `risiko`
  ADD CONSTRAINT `risiko_ibfk_1` FOREIGN KEY (`id_master_risiko`) REFERENCES `master_risiko` (`id_master_risiko`) ON DELETE CASCADE,
  ADD CONSTRAINT `risiko_ibfk_2` FOREIGN KEY (`id_proyek`) REFERENCES `proyek` (`id_proyek`) ON DELETE CASCADE;
